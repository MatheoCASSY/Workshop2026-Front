...
return ( <section className="border-t border-bord-doux py-6 first:border-t-0 first:pt-0"> <div className="mb-3 flex items-baseline gap-3"> <span className="font-mono text-xs text-accent">
...
* Cette suggestion reste pour l'instant une aide visuelle côté front.
  */
  const suggestion = useMemo(() => {
  if (!categorie) return null;


const idsCompetences = COMPETENCES.filter(

  (c) => c.categorie === categorie,
).map((c) => c.id_competence);

return HABILITATIONS.filter((h) =>
  idsCompetences.includes(h.id_competence),
)
  .map((h) => ({
    h,
...
if (envoye) {
return ( <div className="mx-auto max-w-2xl py-16 text-center"> <div className="mx-auto mb-6 flex size-16 items-center justify-center rounded-full border border-succes/40 text-2xl text-succes">
✓ </div>
...
return ( <div className="mx-auto max-w-3xl"> <Link
     href="/incidents"
...
← annuler </Link>
  ...
  >
    ...
    <Etape
      n={2}
      titre="Catégorie"
      aide="Elle détermine quel profil sera sollicité."
    >
    ...
    <div className="mt-2 rounded border border-bord-doux bg-fond px-4 py-3">
      <p className="font-mono text-[11px] uppercase text-faible">
        Attribution proposée
      </p>

      {!categorie ? (
        <p className="mt-1 text-sm text-faible">
          Choisis une catégorie pour voir la suggestion.
        </p>
      ) : suggestion?.m ? (
        <p className="mt-1 text-sm">
          <span className="text-accent">
            {nomComplet(suggestion.m)}
          </span>{" "}
          <span className="text-faible">
            — {suggestion.c?.nom ?? ""} niveau {suggestion.h.niveau},
            disponible
          </span>
        </p>
      ) : (
        <p className="mt-1 text-sm text-alerte">
          Aucun profil « {LIBELLE_CATEGORIE[categorie]} » disponible :
          l&apos;incident ira dans la file d&apos;attente.
        </p>
      )}
    </div>
    ...
    >
      {envoiEnCours
        ...
        : complet
          ...
          : "Titre, description et catégorie requis"}
    </button>
  </form>
</div>
